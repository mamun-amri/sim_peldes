 	...
 	<div class="col-xs-12">
 				...
 				<div class="input-group">
 					...
 					<span class="input-group-btn">
 						<?php
							if ($q <> '') {
							?>
 							<a href="<?php echo site_url('pengajuan_kk'); ?>" class="btn btn-default btn-sm">Reset</a>
 						<?php
							}
							?>
		...
		<th>Kd Post</th> -->
			...
			<td><?php echo $pengajuan->kd_post ?></td> -->
 					...
 					<td><?php echo $pengajuan->lampiran ?> <a href="<?= base_url('pengajuan_kk/view/') . $pengajuan->lampiran ?>" target="_blank" class="badge badge-primary badge-xs"><i class="fa fa-eye" aria-hidden="true"></i></a></td>
 					<td><?php echo $pengajuan->acc_rt ?></td>
 					<td><?php echo $pengajuan->acc_rw ?></td>
 					<td><?php echo $pengajuan->acc_kepdes ?></td>
 					<td style="text-align:center" width="200px">
 						<a href="<?= base_url('pengajuan_kk/acc/') . $pengajuan->id ?>" class="btn btn-success btn-xs" onclick="return confirm('Yakin Acc Data?')"><i class="fa fa-check-circle" aria-hidden="true"></i></a>
 						<a href="<?= base_url('pengajuan_kk/tolak/') . $pengajuan->id ?>" class="btn btn-danger btn-xs" onclick="return confirm('Yakin Tolak Data?')"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
 						<?php $level = $this->session->userdata('id_user_level');
							if ($level == 1 && $pengajuan->acc_rt == "acc" && $pengajuan->acc_rw == "acc" && $pengajuan->acc_kepdes == "acc") {
								echo "<a href=\"" . base_url('pengajuan_kk/cetak/') . $pengajuan->id . "\" target=\"_blank\" class=\"btn btn-secondary btn-xs\" onclick=\"return confirm('Yakin Cetak Data?')\"><i class=\"fa fa-print\" aria-hidden=\"true\"></i></a>";
							}
							?>
 						<?php
							echo anchor(site_url('pengajuan_kk/read/' . $pengajuan->id), '<i class="fa fa-eye" aria-hidden="true"></i>', 'class="btn btn-primary btn-xs"');
							echo '  ';
							// echo anchor(site_url('pengajuan_kk/update/'.$pengajuan->id),'<i class="fa fa-pencil-square-o" aria-hidden="true"></i>','class="btn btn-success btn-xs"'); 
							// echo '  '; 
							// echo anchor(site_url('pengajuan_kk/delete/'.$pengajuan->id),'<i class="fa fa-trash-o" aria-hidden="true"></i>','class="btn btn-danger btn-xs" Delete','onclick="return confirm(\'Yakin Hapus Data?\')"'); 
							?>
 						<!-- <a href="<?= base_url('pengajuan_kk/delete/') . $pengajuan->id ?>" class="btn btn-danger btn-xs" onclick="return confirm('Yakin Hapus Data?')"><i class="fa fa-trash-o" aria-hidden="true"></i></a> -->
 					</td>
 				</tr>
 			<?php
					}
