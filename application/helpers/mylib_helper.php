...
if ( ! function_exists('generatehtml'))
{
    ...
    {
         $CI =& get_instance();
         $query2     = "  SELECT * FROM akademik_tahun_akademik WHERE status='1'";
    ...
    {
        $CI =& get_instance();
        $where  =   array(  'tahun_akademik'=>$tahun_akademik,
    ...
    function chek_bayar($nim,$jenis_bayar,$kode)
    {
        ...
        {
            $sql="SELECT sum(jumlah) as total from keuangan_pembayaran where nim='$nim' and jenis_bayar_id=$jenis_bayar";
    ...
    function status_registrasi($tahun_semester,$nim,$field)
    {
        $CI     =   & get_instance();
        $data=$CI->db->get_where('akademik_registrasi',array('nim'=>$nim,'tahun_akademik'=>$tahun_semester));
        if($data->num_rows()<0)
        {
            return '';
        }
